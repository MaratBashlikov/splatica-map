#!/bin/bash
set -e

echo "=== Splatica Map Server Setup ==="

# Check Node.js
if ! command -v node &> /dev/null; then
    echo "Node.js not found. Installing Node.js..."
    curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
    apt-get install -y nodejs
fi

echo "Node.js version: $(node --version)"
echo "npm version: $(npm --version)"

# Check if git is installed
if ! command -v git &> /dev/null; then
    echo "Installing git..."
    apt-get update
    apt-get install -y git
fi

# Install PM2 for process management
if ! command -v pm2 &> /dev/null; then
    echo "Installing PM2..."
    npm install -g pm2
fi

# Clone or update repository
REPO_DIR="/opt/splatica-map"
if [ -d "$REPO_DIR" ]; then
    echo "Repository exists, updating..."
    cd "$REPO_DIR"
    git pull
else
    echo "Cloning repository..."
    mkdir -p /opt
    cd /opt
    git clone https://github.com/MaratBashlikov/splatica-map.git
    cd "$REPO_DIR"
fi

# Install dependencies
echo "Installing dependencies..."
npm install

# Check if .env.local exists, if not create from template
if [ ! -f .env.local ]; then
    echo "Creating .env.local file..."
    cat > .env.local << 'EOF'
DATABASE_URL="file:./prisma/dev.db"
NEXT_PUBLIC_MAPBOX_TOKEN="your_mapbox_token_here"
ADMIN_PASSWORD="change_me_secure_password"
PORT=8080
EOF
    echo "⚠️  Please edit .env.local and set your MAPBOX_TOKEN and ADMIN_PASSWORD"
fi

# Setup database
echo "Setting up database..."
npx prisma generate
npx prisma db push

# Build the application
echo "Building application..."
npm run build

# Stop existing PM2 process if running
pm2 stop splatica-map 2>/dev/null || true
pm2 delete splatica-map 2>/dev/null || true

# Update package.json start script to use PORT from env
echo "Updating start script to use PORT environment variable..."
# The PORT env var is automatically used by Next.js, so we just need to set it

# Stop existing PM2 process if running
pm2 stop splatica-map 2>/dev/null || true
pm2 delete splatica-map 2>/dev/null || true

# Start with PM2 on port 8080
echo "Starting application with PM2 on port 8080..."
cd "$REPO_DIR"
PORT=8080 pm2 start npm --name "splatica-map" -- start
pm2 save
pm2 startup

echo "=== Setup Complete ==="
echo "Application should be running on port 8080"
echo "Check status: pm2 status"
echo "View logs: pm2 logs splatica-map"

